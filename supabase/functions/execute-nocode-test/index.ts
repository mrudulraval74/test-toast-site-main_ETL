...
serve(async (req) => {
  ...
  try {
    const { testId, projectId, executionId: execId, baseUrl, steps, datasetName, datasetVariables } = await req.json();
    executionId = execId;

    console.log('Starting test execution:', { testId, projectId, executionId, datasetName });
    if (datasetVariables) {
      console.log('Dataset variables provided:', Object.keys(datasetVariables));
    }
    ...
    // Pre-populate with dataset variables if provided (for data-driven testing)
    if (datasetVariables && typeof datasetVariables === 'object') {
      console.log('Pre-populating variables from dataset:', datasetName || 'unnamed');
      for (const [key, value] of Object.entries(datasetVariables)) {
        if (typeof value === 'string') {
          storedVariables.set(key, value);
          console.log(`Dataset variable: {{${key}}} = "${value}"`);
        }
      }
    }
...
async function executeStep(
  ...
  switch (type) {
    ...
    case 'visualRegressionElement': {
      ...
      const useMasks = extraData?.useMasks === true || extraData?.useMasks === 'true';
      ...
      if (baseline.baseline_type === 'storage' && baseline.baseline_storage_path) {
        ...
        if (downloadError || !fileData) {
          ...
          if (baseline.baseline_image) {
            ...
            throw new Error('Failed to load baseline image from storage');
          }
        } else {
      ...
      // Get masks if enabled - ensure it's always an array
      const masks: MaskRegion[] = useMasks && Array.isArray(baseline.masks) ? baseline.masks : [];
      
      console.log(`Visual regression comparison starting:`);
      console.log(`  - Current image length: ${currentBase64.length}`);
      console.log(`  - Baseline image length: ${baselineBase64.length}`);
      console.log(`  - Images are same: ${currentBase64 === baselineBase64}`);
      console.log(`  - Threshold: ${effectiveThreshold * 100}%`);
      console.log(`  - Masks enabled: ${useMasks}, Masks count: ${masks.length}`);
      
      // Pixel comparison with optional mask support
      ...
      if (diffResult.mismatchPercentage > effectiveThreshold * 100) {
        ...
        throw new Error(`Visual regression failed: ${diffResult.mismatchPercentage.toFixed(2)}% difference (threshold: ${(effectiveThreshold * 100).toFixed(2)}%)${masks.length > 0 ? ` with ${masks.length} mask(s) applied` : ''}`);
      }
      ...
      console.log(`Visual regression passed: ${diffResult.mismatchPercentage.toFixed(2)}% difference${masks.length > 0 ? ` (${masks.length} masks applied)` : ''}`);
      break;
    }
...
// Simple pixel-based image comparison with optional mask support
// Masks are arrays of {x, y, width, height} as percentages (0-100)
...
async function compareImages(
  ...
  // Use a canvas-based approach to decode PNG and compare actual pixel data
  // Import pngs library for proper PNG decoding in Deno
  ...
  try {
    ...
    // Fallback: if images can't be decoded, treat as 100% different
  ...
  const currentPixels = currentImage.image; // RGBA format, 4 bytes per pixel
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      // Check if this pixel is within any mask region
      let isMasked = false;
      ...
      // Get pixel index (4 bytes per pixel: R, G, B, A)
      ...
      // Consider pixels different if any channel differs by more than 5 (to handle minor compression artifacts)
  ...
  console.log(`Pixel comparison: ${differentPixels} different pixels out of ${totalComparedPixels} compared (${masks.length} masks applied) = ${mismatchPercentage.toFixed(2)}%`);
