...
async function handleRegister(req: Request): Promise<Response> {
  try {
    ...
    const { projectId, agentId, agentName, agentType, browsers, capacity } = body;

    if (!projectId || !agentName) {
      return new Response(
        JSON.stringify({ error: 'projectId and agentName are required' }),
    ...
    const { data: agent, error } = await supabase
      ...
      .insert({
        ...
        agent_id: agentId || `agent-${Date.now()}`,
        agent_type: agentType || 'selenium', // Store agent type (selenium, playwright, performance, etl)
    ...
    await logActivity(agent.id, 'agent_registered', { agent_name: agentName, agent_type: agentType });

    console.log(`Agent registered: ${agent.id} (${agentName}, type: ${agentType})`);
    ...
    return new Response(
      JSON.stringify({
        ...
        agent_id: agent.id,
        api_key: apiKey,
        apiToken: apiKey, // Also return as apiToken for compatibility
...
// GET /agent-api/agents - List registered agents
async function handleListAgents(req: Request): Promise<Response> {
  try {
    ...
    // Create a client with the user's auth context to enforce RLS
...
async function handleConnections(req: Request, path: string): Promise<Response> {
  try {
    const authHeader = req.headers.get('authorization');
    if (!authHeader) {
      return new Response(
    ...
    if (req.method === 'GET' && (path === '/connections' || path === '/connections/')) {
    ...
    // POST /connections/save - Create or Update
    if (req.method === 'POST' && path === '/connections/save') {
      const body = await req.json();
      // Basic validation
    ...
    if (req.method === 'POST' && path === '/connections/test') {
      // Logic to test connection would go here. 
      // For now, return mock success to unblock UI if actual testing isn't implemented.
      return new Response(
        JSON.stringify({ success: true, message: "Connection test mocked success" }),
        { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
...
Deno.serve(async (req) => {
  ...
  const url = new URL(req.url);
  // Remove prefix if it exists (local dev vs deployed)
  // Handles /agent-api and /functions/v1/agent-api
  ...
  try {
    ...
    // Support action-based routing for existing agent logic
    ...
    // LIST AGENTS (Critical missing endpoint)
    if ((path === '/agents' || path === '/agents/') && req.method === 'GET') {
      return await handleListAgents(req);
    }

    // CONNECTIONS ROUTES
    if (path.startsWith('/connections')) {
      return await handleConnections(req, path);
    }
