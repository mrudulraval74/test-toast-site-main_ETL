...
async function executeJob(job) {
  ...
  try {
    ...
    // Execute each step - handle data-driven testing format
    let actualSteps = job.steps || [];
    let datasetVariables = null;
    let datasetName = null;
    
    // Check if steps contains data-driven testing metadata
    if (actualSteps._datasetVariables) {
      datasetVariables = actualSteps._datasetVariables;
      datasetName = actualSteps._datasetName;
      actualSteps = actualSteps.steps || [];
      log('info', `Data-driven test detected, dataset: ${datasetName}`, { 
        variables: Object.keys(datasetVariables) 
      });
    }
    ...
    if (datasetVariables && typeof datasetVariables === 'object') {
      log('info', `Pre-populating variables from dataset: ${datasetName || 'unnamed'}`);
      for (const [key, value] of Object.entries(datasetVariables)) {
        if (typeof value === 'string') {
          storedVariables.set(key, value);
          log('debug', `Dataset variable: {{${key}}} = "${value}"`);
        }
      }
    }
