...
export const AgentManagement = ({ projectId }: AgentManagementProps) => {
  ...
  const [registerAgentType, setRegisterAgentType] = useState<"selenium" | "playwright" | "performance" | "etl">("selenium");
  const [isRegistering, setIsRegistering] = useState(false);

  // Download dialog state
  const [showDownloadDialog, setShowDownloadDialog] = useState(false);
  const [downloadAgentType, setDownloadAgentType] = useState<"selenium" | "playwright" | "performance" | "etl">("selenium");
...
# API Configuration
API_BASE_URL=https://qfjeicrvelgmyyecvlxo.supabase.co/functions/v1/etl-api
AGENT_API_KEY=${token}
...
`;
        ...
        zip.file("package.json", packageJson);
        zip.file("README.md", readme);
        zip.file("utils/dbConnector.js", dbConnector);
        zip.file("utils/compareEngine.js", compareEngine);
        ...
        const a = document.createElement("a");
        a.href = url;
        a.download = "wispr-etl-agent.zip";
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
      ...
      // Provide specific instructions based on agent type
      const agentMessages = {
            ...
            <>
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={registerAgentType} onValueChange={(value: "selenium" | "playwright" | "performance" | "etl") => setRegisterAgentType(value)}>
        ...
        <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-start gap-3 p-3 border rounded-lg cursor-pointer hover:bg-muted/50 transition-colors">
                <input
                  ...
                  name="downloadAgentType"
                  value="etl"
                  checked={downloadAgentType === "etl"}
                  onChange={() => setDownloadAgentType("etl")}
                  className="mt-1"
                />
                <div>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => downloadAgentPackage(downloadAgentType)}>
              <Download className="h-4 w-4 mr-2" />
              Download {downloadAgentType === "selenium" ? "Selenium" : downloadAgentType === "playwright" ? "Playwright" : downloadAgentType === "etl" ? "ETL" : "Performance"} Agent
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
