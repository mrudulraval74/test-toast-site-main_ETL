...
import { Settings, ArrowLeft, FolderOpen, BarChart3, Brain, Users, Menu, BookOpen, Server, Layers } from "lucide-react";
...
const AppSidebar = ({
  ...
  const [adminMenuVisibility, setAdminMenuVisibility] = useState({
    knowledgeBase: true,
    qaInsights: true,
    aiAnalytics: true,
    roleManagement: true,
  ...
  const fetchAdminMenuVisibility = async () => {
    try {
      ...
      if (error && error.code !== "PGRST116") throw error;
      const settingValue = data?.setting_value as typeof adminMenuVisibility | null;
      if (settingValue) {
        setAdminMenuVisibility(settingValue);
      }
    } catch (error: any) {
  ...
  const fetchCounts = async (projectId: string) => {
    ...
    try {
      ...
      const [storiesResult, defectsResult, plansResult, casesResult] = await Promise.all([
        supabase.from("user_stories").select("id", { count: "exact", head: true }).eq("project_id", projectId),
        supabase
          .from("test_cases")
          .select("id", { count: "exact", head: true })
          .eq("project_id", projectId)
          .eq("status", "failed"),
        supabase.from("saved_test_plans").select("id", { count: "exact", head: true }).eq("project_id", projectId),
    ...
    },
    {
      ...
      items: (() => {
        ...
        const allItems = [
          ...
          { id: "etl-workflow", label: "ETL Workflow" },
  ...
  const mainNavItems = [
    ...
    ...(adminMenuVisibility.architecture ? [{ id: "architecture", label: "Architecture", icon: Layers }] : []),
    ...(adminMenuVisibility.knowledgeBase ? [{ id: "knowledge-base", label: "Knowledge Base", icon: BookOpen }] : []),
    ...(adminMenuVisibility.qaInsights ? [{ id: "qa-insights", label: "QA Insights", icon: BarChart3 }] : []),
    ...(adminMenuVisibility.aiAnalytics ? [{ id: "ai-analytics", label: "AI Analytics", icon: Brain }] : []),
    ...(isAdmin
      ? [
        ...(adminMenuVisibility.roleManagement ? [{ id: "role-manager", label: "Role Management", icon: Users }] : []),
