...
import { TestTube, CheckCircle, XCircle, Clock, Search, Download, Upload, Code2, ChevronDown, ChevronRight, Trash2, Edit3, Save, X, Plus, FolderPlus, Folder, FolderOpen, Flag, MoreHorizontal, Filter, Archive, Copy, Settings, Sparkles, RefreshCw, Brain, Shield, ArrowUpDown, ArrowUp, ArrowDown, Columns } from "lucide-react";
...
export const TestCases = ({
  ...
  // Sorting state
  const [sortColumn, setSortColumn] = useState<'readableId' | 'title' | 'status' | 'priority' | null>('readableId');
  ...
  const generateTestCaseWithAi = async () => {
    ...
    try {
      ...
      if (!data) {
        ...
        const response = await fetch(`${FUNCTIONS_BASE_URL}/generate-test-cases`, {
          method: "POST",
          headers: {
  ...
  const sortedTestCases = [...filteredTestCases].sort((a, b) => {
    ...
    switch (sortColumn) {
      case 'readableId':
        aValue = a.readableId || a.id;
        bValue = b.readableId || b.id;
        break;
      case 'title':
        ...
        break;
      case 'status':
        const statusOrder = { 'passed': 1, 'failed': 2, 'blocked': 3, 'not-run': 4, 'draft': 5 };
  ...
  // Handle column sort
  const handleSort = (column: 'readableId' | 'title' | 'status' | 'priority') => {
  ...
  const getSortIcon = (column: 'readableId' | 'title' | 'status' | 'priority') => {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, statusFilter, priorityFilter, selectedFolderId]);
          ...
          <PopoverContent align="end" className="w-48">
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <Checkbox checked={visibleColumns.title} onCheckedChange={() => toggleColumn('title')} />
                  Title
                </label>
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <Checkbox checked={visibleColumns.automated} onCheckedChange={() => toggleColumn('automated')} />
                  Automated
                </label>
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                ...
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <Checkbox checked={visibleColumns.priority} onCheckedChange={() => toggleColumn('priority')} />
                  Priority
                </label>
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <Checkbox checked={visibleColumns.compliance} onCheckedChange={() => toggleColumn('compliance')} />
                  Compliance
                </label>
                <label className="flex items-center gap-2 text-sm cursor-pointer">
      ...
      <ScrollArea className="flex-1">
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent">
              <TableHead className="w-10">
                <Checkbox checked={paginatedTestCases.length > 0 && paginatedTestCases.every(tc => selectedTestCaseIds.has(tc.id))} onCheckedChange={handleSelectAll} />
              </TableHead>
              {visibleColumns.id && (
                <TableHead className="w-28">
                  <button
                    onClick={() => handleSort('readableId')}
                    className="flex items-center hover:text-foreground transition-colors"
                  >
                    Test Case ID
                    {getSortIcon('readableId')}
                  </button>
                </TableHead>
              )}
              ...
              {visibleColumns.automated && <TableHead className="w-24">Automated</TableHead>}
              ...
              {visibleColumns.compliance && <TableHead className="w-28">Compliance</TableHead>}
          ...
          <TableBody>
            ...
            </TableRow> : sortedTestCases.length === 0 ? <TableRow>
              <TableCell colSpan={visibleColumnCount} className="text-center py-8 text-muted-foreground">
                <TestTube className="h-8 w-8 mx-auto mb-2 opacity-50" />
                No test cases found
              </TableCell>
            </TableRow> : paginatedTestCases.map(testCase => <TableRow key={testCase.id} className="cursor-pointer hover:bg-muted/50" onClick={() => viewTestCaseDetail(testCase)}>
                ...
                <TableCell onClick={e => e.stopPropagation()}>
                  <ComplianceIndicator
                    ...
                    artifact={{
                      ...
                      status: testCase.status,
                      id: testCase.id,
                      readable_id: testCase.readableId,
                    }}
                  />
                </TableCell>
              )}
      ...
      {sortedTestCases.length > 0 && (
        <div className="p-3 border-t border-border flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>per page</span>
            <span className="ml-4">
              Showing {startIndex + 1}-{Math.min(endIndex, sortedTestCases.length)} of {sortedTestCases.length}
            </span>
          </div>
