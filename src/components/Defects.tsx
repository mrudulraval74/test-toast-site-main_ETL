...
export const Defects = ({ onViewChange, projectId }: DefectsProps) => {
  ...
  const loadDefects = async () => {
    ...
    try {
      ...
      if (error) {
        // If table doesn't exist or query fails, show empty state
        console.warn('Defects table query failed:', error);
        setDefects([]);
        setKpiStats({
          totalDefects: 0,
          openDefects: 0,
      ...
      const loadedDefects = defectsData || [];
      setDefects(loadedDefects);

      // Calculate KPI stats from real data
      const totalDefects = loadedDefects.length;
      const openDefects = loadedDefects.filter(d => ['open', 'in-progress'].includes(d.status)).length;
      const resolvedDefects = loadedDefects.filter(d => ['resolved', 'closed'].includes(d.status)).length;
  ...
  const handleRegenerateDefect = async () => {
    ...
    try {
      ...
      if (!configData?.enabled || !configData.config) {
        throw new Error('Azure OpenAI not configured');
      }

      // Regenerate defect report with custom instructions
      const { data, error } = await supabase.functions.invoke('generate-defect-report', {
        body: {
          scenario: formData.scenario,
          steps: formData.logs,
          expectedResult: generatedReport?.expectedResult || "Application should work correctly",
          actualResult: formData.errorSnippet || generatedReport?.actualResult || "System shows unexpected behavior",
          priority: generatedReport?.priority || "medium",
          projectId: projectId,
          azureOpenAiConfig: configData.config,
          customInstructions: customInstructions
        }
      });

      if (error) throw error;
      if (!data.success) throw new Error(data.error || 'Failed to regenerate defect report');

      const updatedReport: GeneratedDefectReport = data.parsedReport ? {
        title: data.parsedReport.title,
        stepsToReproduce: Array.isArray(data.parsedReport.stepsToReproduce) ? data.parsedReport.stepsToReproduce : [data.parsedReport.stepsToReproduce || "Review scenario"],
        actualResult: data.parsedReport.actualResult,
        expectedResult: data.parsedReport.expectedResult,
        priority: data.parsedReport.priority || 'P3',
        severity: data.parsedReport.severity || '3 - Medium',
        screenshot: formData.screenshot
      } : generatedReport!;
